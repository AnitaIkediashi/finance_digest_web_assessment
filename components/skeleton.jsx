  ...
  return (
    <div className="w-full flex flex-wrap gap-6">
      {Array.from({ length: 12 }).map((_, index) => (
        <div key={index} className="flex flex-col gap-4 flex-grow basis-60 min-w-min">
          {/* image */}
          <div className="flex items-center justify-center w-full h-[240px] bg-gray-800 image-animation">
            <svg
              width="48px"
              height="48px"
              viewBox="0 0 24 24"
          ...
          {/* text */}
          <div className="w-full h-[10px] bg-gray-800 rounded-md text-animation" />
        </div>
      ))}
    </div>
  );
